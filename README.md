...
**Core concept:** A recommender system does not "know" what sounds good — it only measures how closely a song's stored attributes match a user's stored preferences, and those measurements are only as meaningful as the features and weights the designer chose.

**Where students struggle:** The hardest moment is when partial matches start competing. Once the perfect song is found the ranking feels obvious, but explaining *why* Gym Hero keeps appearing at #2 for a Happy Pop listener — despite feeling sonically wrong — requires tracing the exact math and confronting the fact that two matched signals can outweigh one mismatched one, regardless of which mismatch matters more to the listener.

**Where AI helped vs. misled:** AI tools were genuinely helpful for explaining the energy formula, generating test profiles, and surfacing edge cases quickly. They were misleading when students asked whether a weight change would "fix" the recommendations — the AI confirmed changes made sense in theory without flagging that a 20-song catalog is too small for weight sensitivity to show up in the top result.

**Guiding without giving the answer:** Ask the student — "If you removed the genre score entirely and only used mood and energy, which songs would appear in the top five for the High-Energy Pop profile, and does that list feel more or less accurate than what you got before?"
